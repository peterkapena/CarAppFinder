...
namespace CarAppFinder.Services
{
    ...
    public class CarService : ICarService
    {
        ...
        public Task<List<Tracker>> Trackers => Context.Trackers.Where(x => !x.Archived).ToListAsync();
        ...
        public async Task<Car> AddCar(Car c)
        {
            ...
            if (!(await Trackers).Any(t => t.Id == c.TrackerId))
            {
        ...
        public async Task<Tracker> AddTracker(Tracker t)
        {
            var tracker = (await Trackers).Where(x => x.Id == t.Id).FirstOrDefault();
